    ...
    document.querySelectorAll('[data-layer-eye]').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        if (layer) {
          ...
          // Atualização cirúrgica in-place (0ms, sem destruir o DOM nem perder scroll)
          const isVis = layer.visible !== false;
          btn.title = isVis ? 'Ocultar Camada' : 'Exibir Camada';
          btn.innerHTML = isVis
          ...
          const layerRow = btn.closest('.cm-ai-layer-row');
          if (layerRow) {
            layerRow.classList.toggle('hidden-layer', !isVis);
          }

          const btnToggleAll = document.getElementById('btn-toggle-all-vis');
          if (btnToggleAll) {
            const allVis = panel.layers.every(l => l.visible !== false);
            btnToggleAll.title = allVis ? 'Ocultar Todas as Camadas' : 'Exibir Todas as Camadas';
            btnToggleAll.textContent = allVis ? '👁️' : '🚫';
          }
        }
      });
    });
    ...
    document.querySelectorAll('[data-feat-eye]').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        if (feat) {
          ...
          btn.title = isVis ? 'Ocultar' : 'Exibir';
          btn.innerHTML = isVis
            ? `<svg viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>`
            : `<svg viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="currentColor" stroke-width="2"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path><line x1="1" y1="1" x2="23" y2="23"></line></svg>`;

          const featRow = btn.closest('.cm-ai-feat-row');
          if (featRow) {
            featRow.classList.toggle('hidden-row', !isVis);
          }
        }
      });
    });
