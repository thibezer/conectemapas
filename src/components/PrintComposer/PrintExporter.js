...
export class PrintExporter {
  ...
  static drawNorthArrowItem(ctx, item, x, y, w, h) {
    ...
    ctx.fillStyle = '#000';
    ctx.beginPath(); ctx.moveTo(0, -r * 0.9); ctx.lineTo(r * 0.25, r * 0.2); ctx.lineTo(0, 0); ctx.closePath(); ctx.fill();

    ctx.fillStyle = '#fff'; ctx.strokeStyle = '#000'; ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.moveTo(0, -r * 0.9); ctx.lineTo(-r * 0.25, r * 0.2); ctx.lineTo(0, 0); ctx.closePath(); ctx.fill(); ctx.stroke();

    ctx.fillStyle = '#000'; ctx.font = `bold ${Math.round(r * 0.35)}px Arial, sans-serif`;
    ctx.textAlign = 'center'; ctx.fillText('N', 0, -r * 0.4);
    ctx.restore();
  }
  ...
  static drawScaleBarItem(ctx, item, x, y, w, h) {
    ...
    ctx.fillStyle = '#000'; ctx.font = `bold ${Math.round(h * 0.22)}px Arial, sans-serif`;
    ctx.textAlign = 'center'; ctx.fillText(`Escala 1:${scaleNum.toLocaleString('pt-BR')}`, x + w / 2, y + h * 0.3);

    const barY = y + h * 0.5, barH = h * 0.2, segW = (w * 0.8) / 4, barX = x + w * 0.1;
    for (let i = 0; i < 4; i++) {
  ...
  static drawTitleBlockItem(ctx, item, x, y, w, h, composer) {
    ...
    const headH = h * 0.22;
    ctx.fillStyle = '#000000'; ctx.fillRect(x, y, w, headH);
    ctx.fillStyle = '#ffffff'; ctx.font = `bold ${Math.round(headH * 0.55)}px Arial, sans-serif`;
    ctx.textAlign = 'center'; ctx.fillText(item.properties?.headerTitle || 'PLANTA TOPOGRÁFICA', x + w / 2, y + headH * 0.7);

    const p = item.properties || {};
    ctx.fillStyle = '#000000'; ctx.font = `bold ${Math.round(h * 0.11)}px Arial, sans-serif`; ctx.textAlign = 'left';
  ...
  static drawLegendItem(ctx, item, x, y, w, h, composer) {
    ...
    ctx.fillStyle = '#000'; ctx.font = `bold ${Math.round(h * 0.16)}px Arial, sans-serif`; ctx.textAlign = 'left';
    ...
    for (const layer of visibleLayers) {
      ...
      ctx.fillStyle = '#000'; ctx.font = `${Math.round(Math.min(12, rowStep * 0.7))}px Arial, sans-serif`;
